...
class DeBruijnGraph (MyGraph):
    ...
    def add_edge(self, o, d): 
        #Adiciona um arco (edge) entre o o e o d
    ...
    def in_degree(self, v):
        #Calcula o número de ligações in ao vértice v
        res = 0
    ...
    def create_deBruijn_graph(self, frags):
        for seq in frags:
            suf = suffix(seq) #Sufixo da seq que é adicionado como um vértice
            self.add_vertex(suf)
            pref = prefix(seq) #Prefixo da seq que é adicionado como um vértice
            self.add_vertex(pref)
            self.add_edge(pref, suf) #Cria uma ligação entre o prefixo e o sufixo

class MyGraph:
    ...
    def check_if_hamiltonian_path(self, p):
        #Verifica se exite um caminho que passe precisamente por todos os nodes uma e uma só vez
    ...
    def search_hamiltonian_path(self):
        #Procura um hamiltonian path para o grafo, começando em todos os nodes
    ...
    def search_hamiltonian_path_from_node(self, start):
        current = start #Nó a processar (começo)
        visited = {start:0} #Dicionário com o caminho/estado dos nós e/ou arcos que já foram visitados (nó/arco + custo)
        path = [start] #Caminho atual
        while len(path) < len(self.get_nodes()):
            nxt_index = visited[current] #Nxt_index é o número do current node
            if len(self.graph[current]) > nxt_index: #Se o tamanho das ligações for maior que das ligações no current node -> é impossivel continuar -> ir para else e dar pop()
                nxtnode = self.graph[current][nxt_index]
                visited[current] += 1 #Aumenta os visitados em uma unidade
                if nxtnode not in path: #Caso em que o nó é adicionado ao caminho
                    path.append(nxtnode)
                    visited[nxtnode] = 0                    
                    current = nxtnode      
            else: #Recua para testar caminhos alternativos - se o comprimento do path for 1 ou menor que 1 é impossível ocorrer um hamiltonian path
                if len(path) > 1: 
                    rmvnode = path.pop() #Remove o último visitado e muda para o anterior
                    del visited[rmvnode]
                    current = path[-1]
                else: 
                    return None
    ...
    def check_balanced_node(self, node):
        #Vê se um node é balanceado
        #Se o número de entradas for igual ao número de saídas (tem de se verificar esta condição)
        return self.in_degree(node) == self.out_degree(node)
        
    def check_balanced_graph(self):
        #Verifica se o grafo é ou não balanceado
    ...
    def is_connected(self):
        #Verifica se todos os vértices do grafo estão conectados a partir de um vértice qualquer 
    ...
    def eulerian_cycle(self):
        def is_in_edges_visit(V, edges_visit): 
            #Serve para ver se ainda existe caminho a partir do vertice V
        ...
        if not self.is_connected() or not self.check_balanced_graph(): 
            return None
        edges_visit = list(self.get_edges()) #Array de tuplos com os vértices e as suas ligações
        res = []
        pss = False #Valor boleano para saber se já não existem caminhos
        t = edges_visit[0] #Tuplo com o vértice e o respetivo caminho
        while edges_visit:
            edges_visit.pop(t) #Elimina o tuplo da lista
            if pss == True:
                res.insert(0, t) #Verifica se t é ou não o começo
                t = res[len(res)-1]
                pss = False
            else:
                res.append(t)
            ...
            for i in edges_visit:
                vertex, path = t
                if is_in_tuple_list(i,  path):
                    tupulo = i
                    break
            vertex, path = tupulo
            vertex_inicial, path_inicial = res[0]
            if path ==  vertex_inicial and not is_in_edges_visit(path, edges_visit): #Verifica se chegou a um ponto sem saída
                pss = True
        cycle = []   
        for i in res:
            vertex,path = i
    ...
    def eulerian_path(self): 
        #Cria um eulerian path para um grafo quase balanceado 
        unb = self.check_nearly_balanced_graph()
        if unb[0] is None or unb[1] is None: 
            return None
        self.graph[unb[1]].append(unb[0])#Cria um caminho de um vértice para o outro de forma a ficarem balanceados
